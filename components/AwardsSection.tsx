...
function TimelineItem({ item, accentColor }: { item: Award; accentColor: string }) {
  return (
    <div className="relative pl-6">
      {/* Timeline dot */}
      <div
        className="absolute left-0 top-1.5 w-3 h-3 rounded-full border-2 border-navy-900"
        style={{ background: accentColor }}
      />
      <p className="font-mono text-xs text-slate-500 mb-0.5">{item.date}</p>
      <p className="text-slate-200 font-semibold text-sm leading-snug mb-0.5">{item.title}</p>
    ...
    <section id="awards" className="py-24 px-4 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <div className="relative pl-3 border-l border-yellow-900/50 space-y-7">
            {awards.map((award, i) => (
              <TimelineItem key={i} item={award} accentColor="#eab308" />
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-mono text-sm uppercase tracking-widest text-violet-400">
              Publications
            </h3>
          </div>
          <div className="relative pl-3 border-l border-violet-900/50 space-y-7">
            {pubs.map((pub, i) => (
              <TimelineItem key={i} item={pub} accentColor="#8b5cf6" />
            ))}
          </div>
        </div>
      </div>
